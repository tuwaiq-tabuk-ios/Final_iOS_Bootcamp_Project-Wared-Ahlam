...
class BookAppointmentViewController: UIViewController {
  ...
  var hospital: HospitalModel = Hospitals().hlist[Constants.R.hospitalIndex]
  ...
  @IBAction func bookButton(_ sender: Any) {
    ...
    guard let time = selectTimeTextField.text, !time.isEmpty else {Constants.alertShow(title: "Error!".Localized(), Msg: "error in getting Time ".Localized(), context: self); return }
    ...
    Firestore.firestore().collection(Constants.Collections.reservations).document(userId).setData(reservation.toDic()) { dbError in
      if dbError != nil {
        ...
        Constants.alertShow(title: "success!".Localized(), Msg: "Reservation done Successfully".Localized(), context: self){result,error in
