...
class ImportantQuestionVC: UIViewController {
  ...
  @IBAction func answerButtonPresswd(_ sender: UIButton) {
    ...
    if sender.titleLabel?.text! == self.question?.proccedAnswer
    {
      selectedIndex += 1
      ...
      Constants.alertShow(title: "Sorry!", Msg: "You cann't Donnate now try in a nother time.", context: self) { result, error in
        if  error == false
        {
          if let next = story.instantiateViewController(withIdentifier: Constants.Storyboard.homeViewControlle) as? HomeViewController{
            next.modalPresentationStyle = .fullScreen
            self.present(next, animated: true, completion: nil)
          }
